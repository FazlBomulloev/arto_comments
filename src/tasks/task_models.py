# Файл src/tasks/task_models.py - обновленные модели
...
class LikeCommentTask(BaseModel):
    ...
    discussion_group_id: Optional[int] = None  # ID группы где искать комментарий
    post_id: Optional[int] = None  # ID поста к которому комментарий
